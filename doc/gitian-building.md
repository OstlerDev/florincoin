...
Clone the git repositories for florincoin and Gitian and then checkout the florincoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the florincoin repository
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/florincoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/florincoin-project/florincoin
...
For example:
```bash
URL=https://github.com/user/florincoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit florincoin=${COMMIT} --url florincoin=${URL} ../florincoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit florincoin=${COMMIT} --url florincoin=${URL} ../florincoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit florincoin=${COMMIT} --url florincoin=${URL} ../florincoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/florincoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/florincoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/florincoin-build.assert
```
...
[florincoin/gitian.sigs](https://github.com/florincoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
request. 
